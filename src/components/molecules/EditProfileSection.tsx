...
type EditProfileSectionProps = {
 ...
 export const EditProfileSection: FC<EditProfileSectionProps> = ({isEditionEnabled, setEditionEnabled}) => {
    ...
    const [form, setForm] = useState<Form>({
      ...
      useEffect(() => {
        if (user.email && user.name && user.phone)  {
          setForm({
            ...
            dob: { value: user.dob ? format(new Date(user.dob), 'yyyy-MM-dd') : '', valid: true, blurred: false },          });
        }
      }, []);
      ...
      async function handleSubmit(event: FormEvent): Promise<void> {
        ...
        if (user._id && isUserValid()) {
          ...
          try {
            ...
            console.error(error);
            setError({
              message: 'We could not change your profile. Make sure the information provided is correct.',
              shouldRefresh: true,
            });
          } finally {
      ...
      function isUserValid(): boolean {
        if (form.name && form.name.valid && user.phone && form.phone.valid && user.email && form.email.valid) {
          return true;
        }
        return false;
      }
